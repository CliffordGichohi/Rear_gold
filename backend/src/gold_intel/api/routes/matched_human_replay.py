...
from gold_intel.application.matched_human_replay import (
    MatchedHumanReplayService,
...
ReplayService = Annotated[
    MatchedHumanReplayService,
    Depends(get_matched_human_replay_service),
]
...
def next_case(service: ReplayService) -> CodexReplaySnapshotResponse:
    ...
    if payload is None:
        raise HTTPException(
            status_code=404,
            detail="All 30 matched human cases are complete; comparison remains sealed",
        )
...
@router.post(
    "/decisions",
    response_model=CodexReplayMutationResponse,
    status_code=201,
)
def seal_decision(
