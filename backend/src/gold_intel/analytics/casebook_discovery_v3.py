...
AUDIT_VERSION = "GOLD_SESSION_BEHAVIOUR_V3_COVERAGE_V0_1"

EXPOSED_2025_START = datetime(2025, 1, 1, tzinfo=UTC)
EXPOSED_2025_END = datetime(2026, 1, 1, tzinfo=UTC)
LOCKED_2026_YTD_START = datetime(2026, 1, 1, tzinfo=UTC)
LOCKED_2026_YTD_END = datetime(2026, 7, 30, tzinfo=UTC)
...
# Non-null counts for spread/volume are explicitly permitted metadata. Every
# column below would reveal a market, macro, release, positioning, probability,
# relationship, or outcome value and is therefore forbidden in audit SQL.
...
def audit_timestamp_only_session_coverage(
    ...
    *,
    ...
    session_date_end_inclusive: date,
) -> dict[str, Any]:
    if session_date_end_inclusive < session_date_start:
        raise ValueError("session_date_end_inclusive must not precede session_date_start")

    available: set[datetime] = set()
    for item in five_minute_open_times:
        if item.tzinfo is None:
            raise ValueError("Timestamp-only coverage inputs must be timezone-aware")
        available.add(item.astimezone(UTC))

    counters = _empty_session_counts()
    missing: dict[str, list[str]] = defaultdict(list)

    current = session_date_start
    while current <= session_date_end_inclusive:
    ...
    return {
        ...
        "session_date_start_inclusive": session_date_start.isoformat(),
        "session_date_end_inclusive": session_date_end_inclusive.isoformat(),
        "definition": {
            ...
            "london_case_prerequisites": ["ASIA", "LONDON"],
            "new_york_case_prerequisites": ["ASIA", "LONDON", "NEW_YORK"],
        },
        "counts": counters,
...
def mt5_xau_files_overlapping(
    ...
    *,
    ...
    for path in sorted(directory.glob("xauusd_1m_ic_markets_mt5_*.csv")):
        ...
        # Filename end is the last observed open_time, so equality overlaps.
...
def timestamp_ranges_from_filenames(paths: Iterable[Path]) -> list[dict[str, Any]]:
    ...
    for path in sorted(paths):
        ...
        ranges.append(
            {
                "name": path.name,
                "declared_first_open_time": _filename_time(
                    match.group("start")
                ).isoformat(),
                "declared_last_open_time": _filename_time(
                    match.group("end")
                ).isoformat(),
            }
        )
...
def overlapping_filename_pairs(paths: Iterable[Path]) -> list[list[str]]:
    parsed: list[tuple[Path, datetime, datetime]] = []
    for path in sorted(paths):
        match = MT5_XAU_FILENAME.fullmatch(path.name)
        if match is None:
            continue
        parsed.append(
            (
                path,
                _filename_time(match.group("start")),
                _filename_time(match.group("end")),
            )
        )
    pairs: list[list[str]] = []
    for index, (left_path, left_start, left_end) in enumerate(parsed):
        for right_path, right_start, right_end in parsed[index + 1 :]:
            if left_start <= right_end and right_start <= left_end:
                pairs.append([left_path.name, right_path.name])
    return pairs
...
def _window_complete(
    ...
    *,
    ...
    return all(
        window_start + timedelta(minutes=5 * offset) in available
