#!/usr/bin/env python3
"""Seal the interaction/display-only Replay V1 UI Amendment B."""
...
PREDECESSOR = ROOT / "research_manifests" / "gold_blind_discretionary_replay_v1_prelabel_ui_amendment_a_freeze.json"
AMENDMENT = ROOT / "GOLD_BLIND_DISCRETIONARY_REPLAY_V1_UI_AMENDMENT_B.md"
...
PAGE = ROOT / "frontend" / "src" / "app" / "replay" / "page.tsx"
GLOBALS = ROOT / "frontend" / "src" / "app" / "globals.css"
STATE = ROOT / "research_artifacts" / "gold_blind_discretionary_replay_v1" / "prelabel_ui_amendment_b_state.json"
FREEZE = ROOT / "research_manifests" / "gold_blind_discretionary_replay_v1_prelabel_ui_amendment_b_freeze.json"
...
WORKSPACE_MARKERS = (
    ...
    "Long position",
    "Short position",
    "Visible history replay controls",
    "Return to checkpoint",
    "decision locking disabled",
    "#FFFFFF",
    "#089981",
    "#F23645",
    "#2962FF",
...
LAB_MARKERS = (
    "Point-in-time fundamental summary",
    "Retracement setup checklist",
    "positionPlanToExecution",
    "Return the chart replay to the frozen decision checkpoint",
)
...
def main() -> int:
    if STATE.exists() or FREEZE.exists():
        raise RuntimeError("Append-only UI Amendment B state or freeze already exists")
    if LEDGER.exists():
        raise RuntimeError("Human labeling has begun; UI Amendment B is refused")

    predecessor = json.loads(PREDECESSOR.read_text(encoding="utf-8"))
    if predecessor.get("status") != "SEALED_READY_FOR_HUMAN_LABELING_UI_AMENDMENT_A":
        raise RuntimeError("The UI Amendment A predecessor has the wrong status")
    ...
    if missing := [marker for marker in WORKSPACE_MARKERS if marker not in workspace_text]:
        raise RuntimeError(f"Required chart-workspace markers are missing: {missing}")
    ...
    gates = {
        ...
        "research_data_backend_and_population_files_unchanged": all(path.startswith("frontend/") for path in differing),
        "amendment_frozen_before_implementation": AMENDMENT.is_file(),
        "light_route_scoped": "replay-light" in page_text and ".replay-light .field" in globals_text,
        "requested_chart_tools_present": all(marker in workspace_text for marker in WORKSPACE_MARKERS[:5]),
        "safe_visible_history_replay_present": "sourceBars.slice" in workspace_text and "decision locking disabled" in workspace_text,
        "position_mapping_rejects_out_of_bounds": "the frozen maximum is 2.00 ATR" in lab_text and "the frozen range is 0.25–3.00 ATR" in lab_text,
        "point_in_time_fundamental_summary_present": "Point-in-time fundamental summary" in lab_text,
    ...
    if not all(gates.values()):
        raise RuntimeError(f"UI Amendment B gates failed: {gates}")

    state = {
        "version": "GOLD_BLIND_DISCRETIONARY_REPLAY_V1_PRELABEL_UI_AMENDMENT_B_STATE_1_0",
        "created_at": datetime.now(UTC).isoformat(),
        "verdict": "PASS_PRELABEL_UI_AMENDMENT_B",
        ...
        "new_or_newly_sealed_ui_files": [
            file_record(AMENDMENT),
            file_record(WORKSPACE),
            file_record(WORKSPACE_TEST),
            file_record(GLOBALS),
        ],
        "tests": {
    ...
    freeze = {
        "version": "GOLD_BLIND_DISCRETIONARY_REPLAY_V1_PRELABEL_UI_AMENDMENT_B_FREEZE_1_0",
        "sealed_at": datetime.now(UTC).isoformat(),
        "status": "SEALED_READY_FOR_HUMAN_LABELING_UI_AMENDMENT_B",
